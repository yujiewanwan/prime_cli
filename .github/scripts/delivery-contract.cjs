...
function validateIssue(issue, deliveryExpected = false) {
  const labels = (issue.labels || []).map(l => typeof l === 'string' ? l : l.name);
  if (!deliveryExpected && !labels.includes('类型:交付') && !trace(issue.body || '').parent) return [];
  ...
  if (types.length !== 1 || types[0] !== '类型:交付') errors.push('交付必须恰好一个类型标签，且为类型:交付。');
  ...
  const scope = body.match(/^#{2,3}\s*(?:交付范围|范围)\s*\n([\s\S]*?)(?=^#{1,3}\s|$(?![\s\S]))/m)?.[1] || '';
  if (!scope.trim()) errors.push('交付范围章节必须有非空内容。');
  const criteria = body.match(/^#{2,3}\s*(?:完成标准|验收标准)\s*\n([\s\S]*?)(?=^#{1,3}\s|$(?![\s\S]))/m)?.[1] || '';
  if (!/^\s*[-*]\s*\[[ xX]\]\s+\S/m.test(criteria)) errors.push('完成标准章节必须有至少一个非空勾选项。');
  return errors;
}

function validatePullRequest(messages, body, issue) {
  ...
  for (const message of messages) {
    if (/\b(?:close[sd]?|fix(?:e[sd])?|resolve[sd]?)\s+(?:#\d+|[^\s]+\/[^\s]+#\d+|https:\/\/github\.com\/[^\s]+\/issues\/\d+)/i.test(message)) errors.push('提交消息禁止关闭引用；只在 PR 正文按单 PR/多 PR 规则关联交付。');
  }
  if (issue) {
    const phases = (issue.labels || []).map(l => typeof l === 'string' ? l : l.name).filter(l => l.startsWith('状态:'));
    if (issue.state !== 'open' || phases.length !== 1 || !['状态:开发中', '状态:待合并'].includes(phases[0])) errors.push('PR 只能关联 Open 且处于开发中或待合并的交付。');
...
async function runPullRequest({ github, context, core }) {
  ...
  const errors = validatePullRequest(messages, body, issue);
  if (errors.length) core.setFailed(errors.join('\n'));
}
