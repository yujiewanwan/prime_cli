...
export function registerWechatOfficialCommands(program: Command): void {
  ...
  articles
    ...
    .action(async (options: AccountsOptions) => {
      ...
      const data = await client.get(
        `/api/wechat-official/articles/accounts?${params.toString()}`,
      );
      console.log(JSON.stringify(data, null, 2));
    });
  ...
  articles
    ...
    .action(async (options: ByFakeidOptions) => {
      ...
      const client = await createAuthorizedClient();
      const data = await client.get(
        `/api/wechat-official/articles/by-fakeid?${params.toString()}`,
      );
      console.log(JSON.stringify(data, null, 2));
    });
  ...
  credentials
    ...
    .hook("preAction", requireRole("SUPER_ADMIN"))
    .action(async (options: CredentialsUpdateOptions) => {
...
function normalizeFakeids(value: string): string {
  const fakeids = value
    .split(",")
    .map((item) => item.trim())
...
function getTimeWindow(options: ByFakeidOptions): {
  startTime: number;
  endTime: number;
} {
...
function unixDayWindow(date: string): { startTime: number; endTime: number } {
