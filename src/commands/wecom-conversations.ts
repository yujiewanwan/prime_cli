...
const ANALYSIS_STATUSES = ["PENDING", "SUCCESS", "FAILED", "SKIPPED"] as const;
const CONVERSATION_TYPES = ["ROOM", "SINGLE"] as const;

export function registerWecomConversationsCommands(program: Command): void {
  ...
  wecomConversations
    ...
    .option(
      ...
      "Conversation type: ROOM or SINGLE",
      (value: string) =>
        normalizeEnumOption(value, "Conversation type", CONVERSATION_TYPES),
    )
    .option(
      "--limit <limit>",
      "Maximum conversations to return",
      (value: string) =>
        parseIntegerOption(value, "Limit", 50, { min: 1, max: 200 }),
    )
    .hook("preAction", requireRole("SUPER_ADMIN"))
    .action(async (options: PendingOptions) => {
  ...
  wecomConversations
    ...
    .requiredOption(
      ...
      "Conversation ID",
      (value: string) =>
        parseIntegerOption(value, "Conversation ID", 1, { min: 1 }),
    )
    .hook("preAction", requireRole("SUPER_ADMIN"))
    .action(async (options: ContextOptions) => {
      const client = await createAuthorizedClient();
      const data = await client.get(
        `/api/agent/wecom-conversations/${options.conversationId}/analysis-context`,
      );
      console.log(JSON.stringify(data, null, 2));
    });
  ...
  wecomConversations
    ...
    .requiredOption(
      "--last-analyzed-seq <seq>",
      "Last analyzed message seq",
      (value: string) =>
        parseIntegerOption(value, "Last analyzed seq", 0, { min: 0 }),
    )
    ...
    .option(
      ...
      "Analysis status: PENDING, SUCCESS, FAILED, or SKIPPED",
      (value: string) =>
        normalizeEnumOption(value, "Analysis status", ANALYSIS_STATUSES),
    )
    .hook("preAction", requireRole("SUPER_ADMIN"))
    .action(async (options: ResultOptions) => {
      ...
      const data = await client.post(
        `/api/agent/wecom-conversations/${options.conversationId}/analysis-result`,
        body,
      );
      console.log(JSON.stringify(data, null, 2));
    });
...
function normalizeEnumOption<const T extends string>(
  value: string,
  name: string,
